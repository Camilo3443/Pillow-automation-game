...
def loadheroestosendhome():
    ...
    print('>>---> %d Heroes that must be recharged.' % len(heroes))
    return heroes
...
def clickgreenbarbuttons():
    ...
    if len(not_working_green_bars) > 0:
        ...
        logger('Clicking on %d hero' % len(not_working_green_bars))
    ...
    for (x, y, w, h) in not_working_green_bars:
        ...
        if hero_clicks_cnt > 20:
            logger('There were too many clicks on heroes, try to increase the go_to_work_btn threshold')
            return
...
def refreshheroespositions():
    logger('Updating heroes position')
...
def login():
    ...
    if login_attempts > 3:
        logger('Too many login attempts, updating...')
        login_attempts = 0
...
def sendheroeshome():
    ...
    if n == 0:
        ...
        return
    print(' %d Heroes that must be sent for recharge found.' % n)
    ...
    for position in heroes_positions:
        if not ishome(position, go_home_buttons):
            ...
            if not isworking(position, go_work_buttons):
                print('Hero is not working, sending to recharge.')
                movetowithrandomness(go_home_buttons[0][0] + go_home_buttons[0][2] / 2, position[1] + position[3] / 2,
                                     1)
                pyautogui.click()
            else:
                print('Hero is working, will not be sent to recharge.')
        else:
            print('Hero is already recharged, or the house is full.')
...
def refreshheroes():
    ...
    if c['select_heroes_mode'] == 'full':
        logger('Sending full-energy heroes to work', 'green')
    elif c['select_heroes_mode'] == 'green':
        logger('Enviando heróis com a energia verde para o trabalho', 'green')
    else:
        logger('Sending Heroes with Green Energy to Work ', 'green')
...
def gobalance():
    ...
    time.sleep(2)
    enviar = ('Your Bcoins balance at %s' % curwind)
    telsendtext(enviar)
...
def main():
    ...
    if len(windows) >= 1:
        ...
        telsendtext(
            '🔌 Bot initialized in %d accounts.' % len(windows))
